...
public:
    ...
    // begins.  Resume checkpoints record the start of a line rather than the
    ...
    // Size of the backing file, for channels that have one.  A resume
    // checkpoint records it so a job file that changed since the checkpoint
