...
static void poll_once() {
    sync_console_report_interval();
        ...
        // Rate-limited internally, and a no-op when no job is running.
        JobResume::poll();
                ...
                if (Channel* channel = Job::channel(); channel && channel->pending_processing_refs() == 0) {
                    ...
                    switch (status) {
                        ...
                        case Error::Eof:
                            ...
                            // Info, not debug: "why did my job stop?" is
                            ...
                            break;
                        default: {
                            // To every channel rather than just the job's
                            // leader, which may itself be gone - that is
                            // exactly the case where the user most needs to
                            // know why the job ended.
                            log_error(static_cast<int>(status) << " (" << errorString(status) << ") in " << channel->name()
...
void protocol_main_loop() {
    ...
    for (;; vTaskDelay(1)) {
        ...
        if (xQueueReceive(cmd_queue, &item, 0)) {
            ...
            // Where the line about to be parsed began.  gc_execute_line() copies
            // this into the planner block so a resume checkpoint can read the
            // file position of the block the machine is actually executing.
